...
func (a *App) Router() http.Handler {
	...
	r.Group(func(pr chi.Router) {
		...
		// v2.13: trojan relays + hedioum wizards + tuning/firewall status
		pr.Get("/api/tunnels/trojan/relays", a.handleListTrojanRelays)
		pr.Get("/api/tunnels/trojan/ingresses", a.handleListTrojanIngresses)
		pr.Get("/api/tunnels/hedioum/egress", a.handleHedioumEgressIP)
		pr.Get("/api/tuning/bbr", a.handleBBRStatus)
		pr.Get("/api/firewall", a.handleFirewallStatus)
		// hedioum-suite v3 extras (reads)
		pr.Get("/api/tunnels/trojan/relays/{id}/verify", a.handleTrojanRelayVerify)
		pr.Get("/api/hedioum/probe", a.handleHedioumProbe)
		pr.Get("/api/hedioum/speedtest", a.handleHedioumSpeedtest)
		pr.Get("/api/hedioum/check-ip", a.handleHedioumCheckIP)
		pr.Get("/api/node-hygiene", a.handleNodeHygiene)
		...
		// v2.13: trojan suite / tuning / firewall on remote nodes
		pr.Get("/api/nodes/{id}/trojan/relays", a.handleNodeTrojanRelays)
		pr.Get("/api/nodes/{id}/trojan/ingresses", a.handleNodeTrojanIngresses)
		pr.Get("/api/nodes/{id}/tuning/bbr", a.handleNodeBBRStatus)
		pr.Get("/api/nodes/{id}/firewall", a.handleNodeFirewallStatus)
		...
		pr.Get("/api/tunnels/pingtunnel", a.handlePingTunnelStatus)
		pr.Post("/api/tunnels/pingtunnel/install", a.Auth.RequireRole("admin", a.handlePingTunnelInstall))
		pr.Post("/api/tunnels/pingtunnel", a.Auth.RequireRole("admin", a.handlePingTunnelCreate))
		pr.Delete("/api/tunnels/pingtunnel/{unit}", a.Auth.RequireRole("admin", a.handlePingTunnelDelete))
		pr.Post("/api/tunnels/pingtunnel/core-remove", a.Auth.RequireRole("admin", a.handlePingTunnelCoreRemove))
		// v2.13: trojan suite mutations + hedioum wizards + tuning/firewall writes
		pr.Post("/api/tunnels/trojan/bridge/validate", a.Auth.RequireRole("operator", a.handleTrojanBridgeValidate))
		pr.Post("/api/tunnels/trojan/bridge/apply", a.Auth.RequireRole("operator", a.handleTrojanBridgeApply))
		pr.Post("/api/tunnels/trojan/ingress/apply", a.Auth.RequireRole("operator", a.handleTrojanIngressApply))
		pr.Post("/api/tunnels/hedioum/setup-foreign", a.Auth.RequireRole("admin", a.handleHedioumSetupForeign))
		pr.Post("/api/tunnels/hedioum/setup-iran", a.Auth.RequireRole("admin", a.handleHedioumSetupIran))
		pr.Post("/api/tuning/bbr/apply", a.Auth.RequireRole("admin", a.handleBBRApply))
		pr.Post("/api/firewall/allow", a.Auth.RequireRole("admin", a.handleFirewallAllow))
		// v2.13: trojan suite purge (destructive — admin only)
		pr.Post("/api/tunnels/trojan/purge", a.Auth.RequireRole("admin", a.handleTrojanPurge))
		// standalone hedioum core (download/install/lifecycle)
		pr.Get("/api/hedioum/core", a.handleHedCoreStatus)
		pr.Post("/api/hedioum/core/install", a.Auth.RequireRole("admin", a.handleHedCoreInstall))
		pr.Post("/api/hedioum/core/service", a.Auth.RequireRole("operator", a.handleHedCoreService))
		pr.Post("/api/hedioum/core/uninstall", a.Auth.RequireRole("admin", a.handleHedCoreUninstall))
		...
		// v2.13: trojan relays/ingresses CRUD (admin — same tier as tunnel relays)
		pr.Post("/api/tunnels/trojan/relays", a.Auth.RequireRole("admin", a.handleCreateTrojanRelay))
		pr.Put("/api/tunnels/trojan/relays/{id}", a.Auth.RequireRole("admin", a.handleUpdateTrojanRelay))
		pr.Delete("/api/tunnels/trojan/relays/{id}", a.Auth.RequireRole("admin", a.handleDeleteTrojanRelay))
		pr.Post("/api/tunnels/trojan/ingresses", a.Auth.RequireRole("admin", a.handleCreateTrojanIngress))
		pr.Put("/api/tunnels/trojan/ingresses/{id}", a.Auth.RequireRole("admin", a.handleUpdateTrojanIngress))
		pr.Delete("/api/tunnels/trojan/ingresses/{id}", a.Auth.RequireRole("admin", a.handleDeleteTrojanIngress))
		...
		// v2.13: trojan/tuning/firewall mutations on remote nodes
		pr.Post("/api/nodes/{id}/trojan/bridge/apply", a.Auth.RequireRole("admin", a.handleNodeTrojanBridgeApply))
		pr.Post("/api/nodes/{id}/trojan/ingress/apply", a.Auth.RequireRole("admin", a.handleNodeTrojanIngressApply))
		pr.Post("/api/nodes/{id}/tuning/bbr/apply", a.Auth.RequireRole("admin", a.handleNodeBBRApply))
		pr.Post("/api/nodes/{id}/firewall/allow", a.Auth.RequireRole("admin", a.handleNodeFirewallAllow))
