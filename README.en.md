...
- **Configuration versioning (v2.7)**: every apply records an immutable snapshot (author, time, deploy result). View, diff (added/removed/changed), download and one-click restore any version — restores are themselves snapshotted, so they can be undone.
...
- **RBAC (v2.7)**: owner / admin / operator / viewer roles enforced on every endpoint (viewers read, operators deploy, admins configure, owners manage users). Users & roles page with last-owner protection.
- **Log management (v2.7)**: bounded tails of allowlisted sources only (agent, panel, nginx, haproxy, xray, hedioum, bridge, syslog) from any node, with live refresh, severity coloring and download — never unrestricted filesystem access.
- **Existing-config import (v2.7)**: on first run, scan the server's live nginx/HAProxy configs and import existing server blocks as (disabled) mappings for review — including upstream pools, redirects and stream forwards.
- **Per-UUID bandwidth limiting (v2.6)**: sync PasarGuard users by Xray UUID, assign reusable bandwidth profiles (or custom values) per user, and enforce download/upload ceilings on nodes with Linux tc — incremental applies, reconciled state, full audit trail. Architecture and honest limitations (UUID vs packet-layer mapping): [docs/BANDWIDTH.md](docs/BANDWIDTH.md).
