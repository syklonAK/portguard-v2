...
- **هر سرور** یک نصب کامل PortGuard است (همان تک‌خطی). هر پنل به‌تنهایی کامل کار می‌کند.
- سروری که می‌خواهید از راه دور مدیریت شود، در کارت **This panel** یک **Node API token** تنظیم می‌کند (دکمه تولید خودکار دارد). این توکن همان چیزی است که master برای احراز هویت به آن وصل می‌شود.
- سرور اصلی (master) در صفحه **Servers** پروفایل هر نود را با host/port و همان توکن ثبت می‌کند.
...
- **Apply** — اجرای خط لوله امن (رندر → اعتبارسنجی → بکاپ → reload با rollback) روی همان سرور از راه دور
...
POST /api/node/apply          — اعمال کانفیگ
GET  /api/node/connections   — لاگ لایو اتصالات
```
همه با هدر `Authorization: Bearer <node_token>`.

> نکته امنیتی: توکن نود فقط از DB خوانده می‌شود و در هیچ پاسخی برگردانده نمی‌شود؛ لیست سرورها در master هم بدون توکن برگردانده می‌شود.
