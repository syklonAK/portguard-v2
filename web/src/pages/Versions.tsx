...
function resultBadge(res: string) {
  ...
  if (!res) return <span className="text-2xs text-slate-400">—</span>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/70">
                {versions.data.map((v, i) => (
                  <tr key={v.version} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                    <td className="px-5 py-2.5 font-mono font-medium">v{v.version}</td>
                    <td className="px-3 py-2.5 text-xs text-slate-500">{new Date(v.created_at).toLocaleString()}</td>
                    ...
                    <td className="px-5 py-2.5">
                      <div className="flex justify-end gap-1">
                        ...
                        <a href={api.downloadVersion(v.version)} download
                          className="rounded-md p-1.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800" title="Download">
        ...
        {diffModal && (
          <div className="space-y-1 font-mono text-xs">
            {diffModal.lines?.length === 0 && (
              <p className="py-6 text-center text-slate-400">No changes between these versions.</p>
            )}
      ...
      <Modal open={viewModal !== null} onClose={() => setViewModal(null)} wide
        ...
        {viewModal && (
          <div className="space-y-3">
            <p className="text-2xs text-slate-400">{viewModal.mappings.length} mappings, {viewModal.relays.length} tunnel relays</p>
